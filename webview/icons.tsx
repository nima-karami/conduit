// Minimal inline-SVG icon set. 16px grid, currentColor stroke.

import * as LucideIcons from 'lucide-react';
import type { JSX as ReactJSX } from 'react';
import type { ArchKind } from '../src/architecture';
import type { ResolvedSessionIcon, SessionIconKind } from '../src/session-icon';

type P = { size?: number; className?: string };
/**
 * Stroke shape (width, cap, join) is NOT set here: it lives in styles.css on `.icon`, where a
 * CSS property beats an SVG presentation attribute, so one theme block retunes every chrome
 * glyph at once. Content-tier Lucide icons carry no `.icon` class and are left alone.
 */
const base = (size = 16, ...classes: (string | undefined)[]) => ({
  width: size,
  height: size,
  viewBox: '0 0 16 16',
  fill: 'none',
  stroke: 'currentColor',
  className: ['icon', ...classes].filter(Boolean).join(' '),
});

/** Every chrome glyph by name — the base set a theme variant may override. */
export const CHROME_ICONS: Record<string, (p: P) => ReactJSX.Element> = {};

/**
 * Neon geometry for the glyphs CSS alone cannot sharpen — a circle stays a circle whatever the
 * cap and join say. A name absent here falls back to the shared geometry, which is the point:
 * most glyphs are already angular and forking all of them would double the maintenance surface.
 * Both variants render and CSS picks one, so the glyph needs no theme plumbing of its own.
 *
 * One substitution rule, borrowed from the theme's own corner notch: a disc becomes a square
 * with its corners cut, and a dot becomes a pip. Curves that carry the MEANING are left alone —
 * rotation, the ⌘ loop, an arch, a hook — because squaring those costs the read at 16px.
 * A dot drawn as a zero-length subpath (`h.01`) is also re-drawn here: it only exists at all
 * because of the round cap, so under a butt cap it disappears.
 */
const NEON_GEOMETRY: Record<string, React.ReactNode> = {
  search: (
    <>
      <path d="M4.3 2.5h5.4l1.8 1.8v5.4l-1.8 1.8H4.3l-1.8-1.8V4.3z" />
      <path d="M10.6 10.6L14 14" />
    </>
  ),
  'zoom-in': (
    <>
      <path d="M4.3 2.5h5.4l1.8 1.8v5.4l-1.8 1.8H4.3l-1.8-1.8V4.3z" />
      <path d="M10.6 10.6L14 14M7 5v4M5 7h4" />
    </>
  ),
  'zoom-out': (
    <>
      <path d="M4.3 2.5h5.4l1.8 1.8v5.4l-1.8 1.8H4.3l-1.8-1.8V4.3z" />
      <path d="M10.6 10.6L14 14M5 7h4" />
    </>
  ),
  folder: <path d="M2 3h4.2l1.3 1.6H14v7.9H2z" />,
  branch: (
    <>
      <rect x="2.6" y="2.6" width="2.8" height="2.8" />
      <rect x="2.6" y="10.6" width="2.8" height="2.8" />
      <rect x="10.6" y="4.1" width="2.8" height="2.8" />
      <path d="M4 5.4v5.2M4 8h8V6.9" />
    </>
  ),
  worktree: (
    <>
      <rect x="3.2" y="2.7" width="2.6" height="2.6" />
      <rect x="3.2" y="10.7" width="2.6" height="2.6" />
      <rect x="10.2" y="6.7" width="2.6" height="2.6" />
      <path d="M4.5 5.3v5.4M4.5 8h5.7" />
      <path d="M13.5 4.5v7" />
    </>
  ),
  history: (
    <>
      <rect x="3.2" y="2.7" width="2.6" height="2.6" />
      <rect x="3.2" y="10.7" width="2.6" height="2.6" />
      <rect x="10.2" y="6.7" width="2.6" height="2.6" />
      <path d="M4.5 5.3v5.4M4.5 8h5.7M7 8l2.5 2.5" />
    </>
  ),
  more: (
    <>
      <rect x="2.6" y="7.1" width="1.8" height="1.8" fill="currentColor" stroke="none" />
      <rect x="7.1" y="7.1" width="1.8" height="1.8" fill="currentColor" stroke="none" />
      <rect x="11.6" y="7.1" width="1.8" height="1.8" fill="currentColor" stroke="none" />
    </>
  ),
  settings: (
    <>
      <rect x="5.9" y="5.9" width="4.2" height="4.2" />
      <path d="M8 1.6v1.7M8 12.7v1.7M3.5 3.5l1.2 1.2M11.3 11.3l1.2 1.2M1.6 8h1.7M12.7 8h1.7M3.5 12.5l1.2-1.2M11.3 4.7l1.2-1.2" />
    </>
  ),
  agent: (
    <>
      <rect x="3" y="5" width="10" height="7" />
      <path d="M8 5V3" />
      <rect x="5.4" y="7.9" width="1.2" height="1.2" fill="currentColor" stroke="none" />
      <rect x="9.4" y="7.9" width="1.2" height="1.2" fill="currentColor" stroke="none" />
    </>
  ),
  server: (
    <>
      <rect x="3" y="3" width="10" height="4" />
      <rect x="3" y="9" width="10" height="4" />
      <rect x="4.9" y="4.4" width="1.2" height="1.2" fill="currentColor" stroke="none" />
      <rect x="4.9" y="10.4" width="1.2" height="1.2" fill="currentColor" stroke="none" />
    </>
  ),
  clock: (
    <>
      <path d="M4.6 2.4h6.8l2.2 2.2v6.8l-2.2 2.2H4.6l-2.2-2.2V4.6z" />
      <path d="M8 4.8V8l2.3 1.5" />
    </>
  ),
  frontend: (
    <>
      <rect x="2.5" y="3" width="11" height="10" />
      <path d="M2.5 6h11" />
      <rect x="3.95" y="3.95" width="1.1" height="1.1" fill="currentColor" stroke="none" />
      <rect x="5.45" y="3.95" width="1.1" height="1.1" fill="currentColor" stroke="none" />
    </>
  ),
  database: (
    <>
      <path d="M8 2l5 2-5 2-5-2z" />
      <path d="M3 4v8l5 2 5-2V4" />
      <path d="M3 8l5 2 5-2" />
    </>
  ),
  worker: (
    <>
      <rect x="5.9" y="5.9" width="4.2" height="4.2" />
      <path d="M8 2.4v1.6M8 12v1.6M13.6 8H12M4 8H2.4M11.8 4.2l-1.1 1.1M5.3 10.7l-1.1 1.1M11.8 11.8l-1.1-1.1M5.3 5.3L4.2 4.2" />
    </>
  ),
  compare: (
    <>
      <rect x="2.6" y="2.6" width="2.8" height="2.8" />
      <rect x="10.6" y="10.6" width="2.8" height="2.8" />
      <path d="M8 4h4v6.6M9.6 2.4L8 4l1.6 1.6" />
      <path d="M8 12H4V5.4M6.4 10.4L8 12l-1.6 1.6" />
    </>
  ),
  'external-system': (
    <>
      <path d="M4.7 2.5h6.6l2.2 2.2v6.6l-2.2 2.2H4.7l-2.2-2.2V4.7z" />
      <path d="M2.6 8h10.8M8 2.5L10.4 8 8 13.5 5.6 8z" />
    </>
  ),
};

/** The names Neon overrides. A key that is not in {@link CHROME_ICONS} is a typo, not a glyph. */
export const NEON_ICON_NAMES = Object.keys(NEON_GEOMETRY);

const glyph = (name: string, paths: React.ReactNode, extraClass?: string) => {
  const Icon = ({ size, className }: P) => {
    const neon = NEON_GEOMETRY[name];
    return (
      <svg {...base(size, extraClass, className)}>
        {neon ? <g className="icon__geo">{paths}</g> : paths}
        {neon ? <g className="icon__geo--neon">{neon}</g> : null}
      </svg>
    );
  };
  CHROME_ICONS[name] = Icon;
  return Icon;
};

/** Glyphs whose fill reads heavy at the line weight of the rest of the set (stars, sparkles). */
const HAIR = 'icon--hair';

export const IconSearch = glyph(
  'search',
  <>
    <circle cx="7" cy="7" r="4.5" />
    <path d="M10.5 10.5L14 14" />
  </>,
);
export const IconPlus = glyph('plus', <path d="M8 3.5v9M3.5 8h9" />);
export const IconZoomIn = glyph(
  'zoom-in',
  <>
    <circle cx="7" cy="7" r="4.5" />
    <path d="M10.5 10.5L14 14M7 5v4M5 7h4" />
  </>,
);
export const IconZoomOut = glyph(
  'zoom-out',
  <>
    <circle cx="7" cy="7" r="4.5" />
    <path d="M10.5 10.5L14 14M5 7h4" />
  </>,
);
export const IconZoomReset = glyph(
  'zoom-reset',
  <>
    <rect x="2.5" y="2.5" width="11" height="11" rx="1.5" />
    <path d="M5.5 5.5h5v5h-5z" />
  </>,
);
export const IconRotate = glyph(
  'rotate',
  <>
    <path d="M13 4.5A5 5 0 103.5 8" />
    <path d="M13 2v3h-3" />
  </>,
);
export const IconChevron = glyph('chevron', <path d="M6 4l4 4-4 4" />);
export const IconChevronDown = glyph('chevron-down', <path d="M4 6l4 4 4-4" />);
export const IconFolder = glyph(
  'folder',
  <path d="M2 4.5A1.5 1.5 0 013.5 3h2.7l1.3 1.6h5A1.5 1.5 0 0114 6.1V11A1.5 1.5 0 0112.5 12.5h-9A1.5 1.5 0 012 11z" />,
);
export const IconPin = glyph(
  'pin',
  <>
    <path d="M9.5 2.5l4 4-2 .5-2.5 2.5-.5 2.5L3.5 7.5 6 7l2.5-2.5z" />
    <path d="M6 10l-3 3" />
  </>,
);
export const IconGraph = glyph(
  'graph',
  <>
    <rect x="2" y="2.5" width="4.5" height="3.5" rx="1" />
    <rect x="9.5" y="10" width="4.5" height="3.5" rx="1" />
    <rect x="9.5" y="2.5" width="4.5" height="3.5" rx="1" />
    <path d="M6.5 4.25h3M6.5 4.25a4 4 0 014 4v3.5" />
  </>,
);
export const IconBranch = glyph(
  'branch',
  <>
    <circle cx="4" cy="4" r="1.6" />
    <circle cx="4" cy="12" r="1.6" />
    <circle cx="12" cy="5.5" r="1.6" />
    <path d="M4 5.6v4.8M4 8h4a4 4 0 004-4" />
  </>,
);
// Worktree marker: a branch fork wrapped by a small bracket, hinting at a linked
// checkout split off the main tree. Decorative (aria-hidden in the indicator).
export const IconWorktree = glyph(
  'worktree',
  <>
    <circle cx="4.5" cy="4" r="1.5" />
    <circle cx="4.5" cy="12" r="1.5" />
    <circle cx="11.5" cy="8" r="1.5" />
    <path d="M4.5 5.5v5M4.5 8h5.5" />
    <path d="M13.5 4.5v7" />
  </>,
);
export const IconSparkle = glyph(
  'sparkle',
  <path d="M8 2.2l1.4 3.6L13 7.2 9.4 8.6 8 12.2 6.6 8.6 3 7.2l3.6-1.4z" />,
  HAIR,
);
export const IconClose = glyph('close', <path d="M4 4l8 8M12 4l-8 8" />);
const _IconArrowUp = glyph('arrow-up', <path d="M8 13V4M4.5 7.5L8 4l3.5 3.5" />);
export const IconSidebar = glyph(
  'sidebar',
  <>
    <rect x="2" y="3" width="12" height="10" rx="1.5" />
    <path d="M6.5 3v10" />
  </>,
);
export const IconSwap = glyph('swap', <path d="M3 6h8l-2-2M13 10H5l2 2" />);

export const IconMore = glyph(
  'more',
  <>
    <circle cx="3.5" cy="8" r="0.9" fill="currentColor" stroke="none" />
    <circle cx="8" cy="8" r="0.9" fill="currentColor" stroke="none" />
    <circle cx="12.5" cy="8" r="0.9" fill="currentColor" stroke="none" />
  </>,
);
export const IconCheck = glyph('check', <path d="M3.5 8.5l3 3 6-7" />);
export const IconSettings = glyph(
  'settings',
  <>
    <circle cx="8" cy="8" r="2.1" />
    <path d="M8 1.6v1.7M8 12.7v1.7M3.5 3.5l1.2 1.2M11.3 11.3l1.2 1.2M1.6 8h1.7M12.7 8h1.7M3.5 12.5l1.2-1.2M11.3 4.7l1.2-1.2" />
  </>,
);
export const IconCommand = glyph(
  'command',
  <path d="M5.5 3.5A1.5 1.5 0 105.5 6.5h5a1.5 1.5 0 100-3 1.5 1.5 0 00-1.5 1.5v5a1.5 1.5 0 11-1.5-1.5h-1" />,
);
export const IconExternal = glyph('external', <path d="M9 3.5h3.5V7M12 4l-5 5M11 9v3.5H3.5V5H7" />);
export const IconCopy = glyph(
  'copy',
  <>
    <rect x="5.5" y="5.5" width="7" height="7" rx="1.2" />
    <path d="M3.5 10.5V4A1.5 1.5 0 015 2.5h5.5" />
  </>,
);
export const IconPaste = glyph(
  'paste',
  <>
    <rect x="3.5" y="3.5" width="9" height="10" rx="1.2" />
    <path d="M6 3.5V2.8a.8.8 0 01.8-.8h2.4a.8.8 0 01.8.8v.7z" />
  </>,
);
export const IconEraser = glyph(
  'eraser',
  <>
    <path d="M6.5 12.5L3 9a1 1 0 010-1.4l4.6-4.6a1 1 0 011.4 0L13 6.9a1 1 0 010 1.4l-4.2 4.2z" />
    <path d="M6.5 12.5h6.5" />
  </>,
);
export const IconDuplicate = glyph(
  'duplicate',
  <>
    <rect x="5.5" y="5.5" width="7" height="7" rx="1.2" />
    <path d="M3.5 10.5V4A1.5 1.5 0 015 2.5h5.5M9 7v3M7.5 8.5h3" />
  </>,
);
export const IconTrash = glyph(
  'trash',
  <path d="M3 4.5h10M6.5 4.5V3.2A.7.7 0 017.2 2.5h1.6a.7.7 0 01.7.7v1.3M4.5 4.5l.6 8a.8.8 0 00.8.7h4.2a.8.8 0 00.8-.7l.6-8" />,
);
export const IconBoard = glyph(
  'board',
  <>
    <rect x="2.5" y="3" width="11" height="10" rx="1.5" />
    <path d="M6 3v10M10 3v10" />
  </>,
);
export const IconPencil = glyph('pencil', <path d="M10.5 2.8l2.7 2.7L6 12.7l-3 .6.6-3z" />);
export const IconReview = glyph(
  'review',
  <>
    <path d="M4 2h5l3 3v6.5A1.5 1.5 0 0110.5 13h-6A1.5 1.5 0 013 11.5v-8A1.5 1.5 0 014 2z" />
    <path d="M9 2v3h3" />
    <path d="M5.5 9.2l1.3 1.3L9.5 7.8" />
  </>,
);
// Commit-history glyph: a node on a vertical lane with a branch fork — reads as "git
// graph / history" beside the branch indicator. Decorative; render aria-hidden.
export const IconHistory = glyph(
  'history',
  <>
    <circle cx="4.5" cy="4" r="1.5" />
    <circle cx="4.5" cy="12" r="1.5" />
    <path d="M4.5 5.5v5" />
    <circle cx="11.5" cy="8" r="1.5" />
    <path d="M4.5 8h2.5a2.5 2.5 0 012.5 2.5M10 8H7" />
  </>,
);
// Compare entry-point glyph (spec 2026-06-30-review-compare-dialog §C/§11): two lanes with
// opposed arrows, visually distinct from History (a fork) and Review (a doc). Lucide's
// GitCompareArrows redrawn on the 16 grid — as a foreign 24-grid glyph it was the one chrome
// icon a theme could not reshape, and it sits in the git band right beside the two it clashed with.
export const IconCompare = glyph(
  'compare',
  <>
    <circle cx="4" cy="4" r="1.6" />
    <circle cx="12" cy="12" r="1.6" />
    <path d="M8 4h2.7a1.3 1.3 0 011.3 1.3V10M9.6 2.4L8 4l1.6 1.6" />
    <path d="M8 12H5.3A1.3 1.3 0 014 10.7V5.6M6.4 10.4L8 12l-1.6 1.6" />
  </>,
);
export const IconRefresh = glyph(
  'refresh',
  <>
    <path d="M12.5 8a4.5 4.5 0 1 1-1.32-3.18" />
    <path d="M12.7 3v2.3h-2.3" />
  </>,
);
export const IconDownload = glyph(
  'download',
  <>
    <path d="M8 3v7.5M4.5 8L8 11.5 11.5 8" />
    <path d="M3.5 13h9" />
  </>,
);
export const IconRefreshCw = glyph(
  'refresh-cw',
  <>
    <path d="M12.5 4.5A5.5 5.5 0 0 0 3 7.5" />
    <path d="M3.5 11.5A5.5 5.5 0 0 0 13 8.5" />
    <polyline points="12.5 2 12.5 5 9.5 5" />
    <polyline points="3.5 14 3.5 11 6.5 11" />
  </>,
);

const IconAgent = glyph(
  'agent',
  <>
    <rect x="3" y="5" width="10" height="7" rx="2" />
    <path d="M8 5V3M6 8.5h.01M10 8.5h.01" />
  </>,
);
const IconSkill = glyph(
  'skill',
  <path d="M8 2.5l1.5 3.1 3.4.5-2.45 2.4.58 3.4L8 9.8 4.97 11.9l.58-3.4L3.1 6.1l3.4-.5z" />,
);
export const IconDoc = glyph(
  'doc',
  <>
    <path d="M4 2.5h5L12 5.5V13a.5.5 0 01-.5.5h-7A.5.5 0 014 13z" />
    <path d="M9 2.5V6h3" />
  </>,
);
const IconHook = glyph(
  'hook',
  <>
    <path d="M8 3v5a2.5 2.5 0 11-2.5 2.5" />
    <path d="M6 3h4" />
  </>,
);
const IconServer = glyph(
  'server',
  <>
    <rect x="3" y="3" width="10" height="4" rx="1" />
    <rect x="3" y="9" width="10" height="4" rx="1" />
    <path d="M5.5 5h.01M5.5 11h.01" />
  </>,
);

export const IconWinMin = glyph('win-min', <path d="M3 8h10" />);
export const IconWinMax = glyph('win-max', <rect x="3.5" y="3.5" width="9" height="9" rx="1" />);
export const IconWinRestore = glyph(
  'win-restore',
  <>
    <rect x="5" y="3" width="8" height="8" rx="1" />
    <path d="M3 6v7h7" />
  </>,
);

export const IconTerminal = glyph(
  'terminal',
  <>
    <rect x="2" y="3" width="12" height="10" rx="1.5" />
    <path d="M4.8 6.3l2 1.7-2 1.7M8.5 10h2.7" />
  </>,
);

export const IconClock = glyph(
  'clock',
  <>
    <circle cx="8" cy="8" r="5.6" />
    <path d="M8 4.8V8l2.3 1.5" />
  </>,
);

// PowerShell-flavoured terminal — NOT the official logo.
const IconPowerShell = glyph(
  'powershell',
  <>
    <rect x="2" y="3" width="12" height="10" rx="1.5" />
    <path d="M5 6l2.4 2-2.4 2M8.6 10.2h2.6" />
  </>,
);

// Generic AI/sparkle mark for Claude-like agents — NOT an official logo.
const IconClaude = glyph(
  'claude',
  <>
    <path d="M7 2.2l1.1 2.9L11 6.2 8.1 7.3 7 10.2 5.9 7.3 3 6.2l2.9-1.1z" />
    <path d="M11.6 9.4l.5 1.4 1.4.5-1.4.5-.5 1.4-.5-1.4-1.4-.5 1.4-.5z" />
  </>,
  HAIR,
);

const _customIcon: Record<string, (p: P) => ReactJSX.Element> = {
  agent: IconAgent,
  skill: IconSkill,
  doc: IconDoc,
  hook: IconHook,
  server: IconServer,
};

/** Icon for a launchable shell/agent, keyed by its AgentDefinition.icon. */
const _agentIcon: Record<string, (p: P) => ReactJSX.Element> = {
  sparkle: IconSparkle,
  terminal: IconTerminal,
};

const SESSION_ICON: Record<SessionIconKind, (p: P) => ReactJSX.Element> = {
  claude: IconClaude,
  powershell: IconPowerShell,
  terminal: IconTerminal,
};

/**
 * Glyph for a session tab (D4). Decorative/aria-hidden — the label carries meaning for
 * assistive tech. Call sites pass a ResolvedSessionIcon from {@link resolveSessionIcon} so
 * iconOverride is respected everywhere (D3). The glyph carries IDENTITY only — status is the
 * card's own dot + word (src/session-dot.ts), so it is never double-coded here.
 */
export function SessionGlyph({ icon, size, className }: P & { icon: ResolvedSessionIcon }) {
  if (icon.type === 'lucide') {
    // lucide-react exports every icon under its PascalCase name.
    const pascalName = icon.name
      .split('-')
      .map((p) => p.charAt(0).toUpperCase() + p.slice(1))
      .join('');
    const LucideIcon = (LucideIcons as Record<string, unknown>)[pascalName] as
      | React.FC<{ size?: number; className?: string }>
      | undefined;
    if (LucideIcon) {
      return (
        <span className="session__icon" aria-hidden>
          <LucideIcon size={size} className={className} />
        </span>
      );
    }
    // Unknown icon name — fall back to a generic terminal glyph rather than rendering nothing.
    const Fallback = SESSION_ICON.terminal;
    return (
      <span className="session__icon" aria-hidden>
        <Fallback size={size} className={className} />
      </span>
    );
  }
  const Icon = SESSION_ICON[icon.kind];
  return (
    <span className="session__icon" aria-hidden>
      <Icon size={size} className={className} />
    </span>
  );
}

// Architecture node-kind glyphs (F4): one per ArchKind, currentColor so the node tints it
// with the kind's design-variable color.
const IconService = glyph(
  'service',
  <>
    <rect x="2.5" y="3" width="11" height="10" rx="1.6" />
    <path d="M6.5 6l3 2-3 2z" />
  </>,
);
const IconGateway = glyph(
  'gateway',
  <>
    <path d="M3 13V7a5 5 0 0110 0v6" />
    <path d="M8 5.5v5M5.8 8.2L8 6l2.2 2.2" />
  </>,
);
const IconFrontend = glyph(
  'frontend',
  <>
    <rect x="2.5" y="3" width="11" height="10" rx="1.6" />
    <path d="M2.5 6h11M4.5 4.5h.01M6 4.5h.01" />
  </>,
);
const IconDatabase = glyph(
  'database',
  <>
    <ellipse cx="8" cy="4" rx="5" ry="2" />
    <path d="M3 4v8c0 1.1 2.24 2 5 2s5-.9 5-2V4" />
    <path d="M3 8c0 1.1 2.24 2 5 2s5-.9 5-2" />
  </>,
);
const IconCache = glyph('cache', <path d="M9 2L4 9h3.5L7 14l5-7H8.5z" />);
const IconQueue = glyph(
  'queue',
  <>
    <path d="M2.5 5h7M2.5 8h7M2.5 11h7" />
    <path d="M11 6.5L13.5 8 11 9.5" />
  </>,
);
const IconWorker = glyph(
  'worker',
  <>
    <circle cx="8" cy="8" r="2.1" />
    <path d="M8 2.4v1.6M8 12v1.6M13.6 8H12M4 8H2.4M11.8 4.2l-1.1 1.1M5.3 10.7l-1.1 1.1M11.8 11.8l-1.1-1.1M5.3 5.3L4.2 4.2" />
  </>,
);
const IconStorage = glyph(
  'storage',
  <>
    <path d="M2.5 5.5L8 3l5.5 2.5v5L8 13l-5.5-2.5z" />
    <path d="M2.5 5.5L8 8l5.5-2.5M8 8v5" />
  </>,
);
const IconLibrary = glyph(
  'library',
  <>
    <rect x="3" y="3" width="3" height="10" rx="0.6" />
    <rect x="6.5" y="3" width="3" height="10" rx="0.6" />
    <path d="M10.2 3.6l2.6.7-2 9.3-2.6-.7z" />
  </>,
);
const IconExternalSystem = glyph(
  'external-system',
  <>
    <circle cx="8" cy="8" r="5.5" />
    <path d="M2.6 8h10.8M8 2.5c1.8 1.6 1.8 9.4 0 11M8 2.5c-1.8 1.6-1.8 9.4 0 11" />
  </>,
);
const IconGroup = glyph(
  'group',
  <rect x="2.5" y="2.5" width="11" height="11" rx="1.6" strokeDasharray="2.4 2" />,
);

/** Distinct glyph per architecture node kind (F4). Decorative; render aria-hidden. */
export const KIND_ICON: Record<ArchKind, (p: P) => ReactJSX.Element> = {
  service: IconService,
  gateway: IconGateway,
  frontend: IconFrontend,
  database: IconDatabase,
  cache: IconCache,
  queue: IconQueue,
  worker: IconWorker,
  storage: IconStorage,
  library: IconLibrary,
  external: IconExternalSystem,
  group: IconGroup,
};
