...
import {
  seedArchitecture, addNode, updateNode, moveNode, removeNode, addEdge, removeEdge,
  ensureChildGraph, breadcrumb, serializeArchitecture, restoreArchitecture,
...
describe('architecture model', () => {
  ...
  it('adds, updates and moves a node', () => {
    ...
    const { doc: d2, id } = addNode(doc, doc.rootGraph, { title: 'Cache', kind: 'data', x: 10, y: 20 });
    doc = d2;
    expect(doc.graphs[doc.rootGraph].nodes.find((n) => n.id === id)).toMatchObject({ title: 'Cache', kind: 'data', x: 10, y: 20 });
    ...
    expect(doc.graphs[doc.rootGraph].nodes.find((n) => n.id === id)).toMatchObject({ x: 99, y: 88 });
  });
  ...
  it('removes a node together with its incident edges', () => {
    ...
    expect(out.graphs[g].edges.some((e) => e.source === 'n-core' || e.target === 'n-core')).toBe(false);
  });
  ...
  it('round-trips through serialize/restore and drops dangling child refs', () => {
    ...
    const blob = JSON.stringify({
      version: 1, rootGraph: 'r',
      graphs: { r: { id: 'r', title: 'R', nodes: [{ id: 'a', title: 'A', kind: 'service', x: 0, y: 0, childGraph: 'ghost' }], edges: [] } },
    });
