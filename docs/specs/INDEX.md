...
| Date | Spec |
|------|------|
| 2026-07-01 | [review-diff-syntax](2026-07-01-review-diff-syntax.md) — per-language **syntax highlighting** for Review Changes diff rows (the primary agent-diff surface): reuse the app's existing `highlight.js` engine (no new dep) in a pure `webview/syntax-highlight.ts` (`highlightLine` → `{text,cls}` segments, monaco→hljs lang map, bounded cache, long-line guard), tokenize only windowed rows, colors from a **single editor-matching palette** via `--syn-*` CSS vars + an app-owned hljs theme (replacing markdown's hardcoded github-dark); add/remove tint + `+`/`-` sign preserved under token colors; unknown lang → plain fallback (FULL, UI) |
| 2026-07-01 | [theme-correctness](2026-07-01-theme-correctness.md) — make all six themes render correctly (esp. Paper + High contrast): Monaco code/diff editors become **theme-aware via per-theme `--editor-*` CSS vars** in styles.css (base from `--editor-base`, dark defaults = today's look) + a **live re-theme on theme switch** (rAF-deferred to dodge the stale-`data-theme` read; code-viewer dep + new diff-viewer effect); audited hardcoded-color fixes (md bold `#fff`, dead `--vscode-input-background` inputs, `--on-accent` buttons, `.codeblock__src`); **D1 (high)**: shared code/terminal surface follows the theme via a `surfaceColor: 'auto'` sentinel so Paper's editor is light out-of-box. FULL |
| 2026-07-01 | [mermaid-export](2026-07-01-mermaid-export.md) — export a rendered mermaid diagram from the zoom overlay: two toolbar actions (**Export SVG** + **Export PNG**) trigger a **renderer-only** download (temporary `<a download>` + `URL.createObjectURL`; no host save-dialog IPC). Pure `webview/mermaid-export.ts` — `svgToBlob` (standalone `image/svg+xml` with an idempotently-injected XML prolog + `xmlns`), `svgToPngBlob` (rasterize via `viewBox`-derived size → data URL → `Image` → `<canvas>` at 2×), `download`, `diagramFilename`. LITE (UI) |
| 2026-07-01 | [markdown-search](2026-07-01-markdown-search.md) — in-rendered-markdown find (Ctrl/Cmd+F): a viewer-scoped find bar (owns-check document-capture, like the file's Ctrl+A — no global-find hijack) that highlights matches via the **CSS Custom Highlight API** (no DOM mutation, so React reconciliation is untouched), shows a `n/total` count, cycles Enter/Shift+Enter (wrap) + scrolls current into view, Esc clears. Pure `md-find.ts` (mirrors `pdf-find.ts`) + TreeWalker range building; reuses `.term-find` chrome. FULL (UI) |
