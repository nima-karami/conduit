...
| Date | Spec |
|------|------|
| 2026-08-27 | [review-supercharge](2026-08-27-review-supercharge.md) — **epic, six lanes (A→B→D→E→C→F)**: **A** editor change decorations vs HEAD (gutter bars/triangles, overview ruler, minimap on by default, live re-diff, `Alt+F5` next/prev); **B** Review keymap (`j k J K m o e`), collapse/expand all, sticky headers, reviewed marks persisted to `userData/review-marks.json` with content-hash pruning, range quick-picks via `git:resolveRange`, ignore-whitespace; **C** search across changed lines + file filter; **D** All/Staged/Unstaged scope; **E** per-hunk stage/unstage/discard (host-built patch from `git diff -U3` filtered by line range, index baseline) + editor change peek; **F** content-anchored line notes in enveloped `.conduit/review-notes.json` + "Send to agent" via the unified terminal bus (bracketed paste) into the Review's session. |
