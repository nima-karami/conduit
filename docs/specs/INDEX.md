...
| 2026-08-28 | [timed-messages](2026-08-28-timed-messages.md) — send a message to a session on a **delay**, a **clock time**, or a repeating **interval** with a required max-repeats, armed from the command bar and shown as a chip over that session's terminal. Host-owned schedules in `userData/timed-messages.json`; a fire is a liveness-checked `pty.input` + a separate `Enter` (never `mgr.touch`), and it requires a **live PTY** — otherwise it **waits**, visibly, until the user starts the session (the host never spawns one). Also **limit-aware auto-resume**: a usage-limit notice in the session's *trailing* output arms `Continue` at the parsed reset time (`autoResumeOnLimit` default `arm`), one per session, labelled `Auto` and cancellable in one click. |
