...
| Date | Spec |
|------|------|
| 2026-08-08 | [mac-topbar-layout](2026-08-08-mac-topbar-layout.md) — macOS-only top bar rework: native traffic lights + back/forward own the left, search stays centered, attention chip + Workspace/Board/Canvas tabs + logo own the right, no `.winctl` on mac at all. Consolidates `isMac` to a single export and fixes a stale `trafficLightPosition` comment/offset. Windows/Linux unaffected; live density/fullscreen re-centering explicitly deferred. |
