...
| Date | Spec |
|------|------|
| 2026-08-08 | [mac-topbar-layout](2026-08-08-mac-topbar-layout.md) — rebasing `macos-window-chrome` onto current main conflicts because main grew the Workspace/Board/Canvas switcher + search pill after the branch was cut. Redesigns the mac-only top bar: traffic lights flush left with back/forward next to them, search stays centered, attention chip + tabs + logo flush right, no `.winctl` on mac at all. Also fixes Nima's PR #2 nits (`isMac` import instead of a third local declaration; stale "44px" `trafficLightPosition` comment/offset). Windows/Linux untouched; live density/fullscreen re-centering explicitly deferred. |
