...
| 2026-08-20 | [renderer-crash-recovery](2026-08-20-renderer-crash-recovery.md) — a renderer OOM/crash left the window permanently **black** while the main process, its sessions and their PTY children were all still alive; quitting to escape it killed every live shell. `render-process-gone` now reloads the window in place (bounded: 3 reloads per 5-minute sliding window, `killed`/`clean-exit` ignored), and the reloaded renderer re-attaches through the existing `term:start` attach path. |
| 2026-08-20 | [commit-review-memory-bounds](2026-08-20-commit-review-memory-bounds.md) — opening a huge monorepo commit in Review OOM'd the renderer: every file in the changeset was line-diffed on arrival with a dense `(n+1)×(m+1)` LCS table (a 1.5 MB lockfile ≈ 13 GB). `diffLines` now trims the common prefix/suffix and, past `MAX_LCS_CELLS`, degrades to an `approx` whole-core replacement — memory is O(n+m) always. Badges come from a host `diff-tree --numstat -z` join instead of a renderer parse; `useCommitFiles` gains the error channel + Retry its range sibling already had; `getCommitDiff` runs under the 10 s diff timeout. |
